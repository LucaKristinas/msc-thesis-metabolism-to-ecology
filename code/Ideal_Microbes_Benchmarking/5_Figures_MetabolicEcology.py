...
# Output
output_path = processed_data_path / "figures/metabolic_ecology"
...
for label, df, mse_df in [
   ...
   # }
    ...
    # Weighted λ means (rounded to 2 decimals)
    means[label] = {
        k: round(np.sum(l * w), 2)
        for k, (l, w) in lambdas[label].items()
    }
    ...
    # vit start
    export_data = {
        f"lambda_best": lambdas[label]['best'][0],
        f"q_best":      lambdas[label]['best'][1],
        f"lambda_min":  lambdas[label]['min'][0],
        f"q_min":       lambdas[label]['min'][1],
        f"lambda_max":  lambdas[label]['max'][0],
        f"q_max":       lambdas[label]['max'][1]
    }
    
    # Create DataFrame
    # Note: If arrays have different lengths, pd.DataFrame will raise an error.
    # We use pd.Series to handle potential length mismatches (it will fill with NaN)
    #output_df = pd.DataFrame({k: pd.Series(v) for k, v in export_data.items()})
    
    # Save to CSV
    #filename = f"{label}s_BiomassFlux_Data.csv"
    #output_df.to_csv(csv_processed_path / filename, index=False)
...
for name, selected_labels, selected_subset, selected_std, gr_value in plot_configs:
    ...
    # Style the separate legend frame
    frame = legend_sep.get_frame()
    frame.set_facecolor('white')
    frame.set_edgecolor('black')
    frame.set_linewidth(0.8)
    frame.set_alpha(1.0)

    legend_fig.savefig(output_path / f"{name}_legend.png", dpi=300, bbox_inches='tight')
    legend_fig.savefig(output_path / f"{name}_legend.svg", bbox_inches='tight')
    plt.close(legend_fig)

    legend.remove()

    # Title and formatting
    ax.set_title(fr"Flux Prediction using {pathway_type} for $\lambda = {gr_value}\,h^{{-1}}$", fontsize=16)
    ax.set_xticks(x_base)
    ax.set_xticklabels(target_reactions_sorted, rotation=90)
    ax.tick_params(axis='x', labelsize=14)
    ax.tick_params(axis='y', labelsize=14)
    ax.set_ylabel("Flux relative to GLC uptake",fontsize=14)
    ax.set_ylim(-0.06, 2)
...
subplot_titles = [
    r"EFMs ($\lambda = 0.1\,h^{-1}$)",
    r"EFMs ($\lambda = 0.2\,h^{-1}$)",
    r"EFVs ($\lambda = 0.1\,h^{-1}$)",
    r"EFVs ($\lambda = 0.2\,h^{-1}$)"
]
...
for idx, (label_str, selected_labels, selected_subset) in enumerate(panel_configs):
    ...
    if idx % 2 == 0:
        ax.set_ylabel("Flux relative to GLC uptake", fontsize=12)

    # Set individual title
    ax.set_title(subplot_titles[idx], fontsize=12, pad=8)
...
ax.text(target_x, y_GR02 + 0.0025, r'$\lambda = 0.2\,h^{-1}$', fontsize=12, color=curve_colors[2])
ax.text(target_x, y_GR01 + 0.0025, r'$\lambda = 0.1\,h^{-1}$', fontsize=12, color=curve_colors[3])
...
for config, ax in zip(curve_configs, axs):
    ...
    ax.text(T_opt_local - x_offset, 0.23, fr'$T = {T_opt_local:.5f}$',
            fontsize=14, color=background_color, ha='right', va='top')
    ...
    ax.tick_params(axis='y', labelsize=14)
    ax.grid(False)
    ax.set_title(f"MSE Curves for {label}", fontsize=16)
...
for config, ax in zip(curve_configs, axs):
    ...
    x_offset = T_opt_local * 0.25
    ax.text(T_opt_local - x_offset, 0.23, fr'$T = {T_opt_local:.5f}$',
            fontsize=12, color=background_color, ha='right', va='top')
    ...
    ax.set_yticks(np.arange(0.05, 0.275, 0.025))
    ax.grid(False)
    ax.set_title(f"MSE Curves for {label}", fontsize=12)
...
titles_c = {
    'EFM': "Growth Rate Distributions predicted by EFMs",
    'EFV': "Growth Rate Distributions predicted by EFVs"
}
...
for i, label in enumerate(['EFM', 'EFV']):
    ax = axs[i]
    ax.set_title(titles_c[label], fontsize=16, pad=10)
    ...
    ax.text(means[label]['max'] - 0.005, 450, f"{means[label]['max']:.2f}", color=color_max_T, ha='right', va='bottom', fontsize=14)
    ax.text(means[label]['min'] + 0.005, 450, f"{means[label]['min']:.2f}", color=color_min_T, ha='left', va='bottom', fontsize=14)
    ax.text(means[label]['best'] - 0.005, 450, f"{means[label]['best']:.2f}", color=color_best_T, ha='right', va='bottom', fontsize=14)
    ...
    # Labels
    if label == 'EFV':
        ax.set_xlabel(r'Growth Rate $\lambda\, \mathrm{in}\ \mathrm{h}^{-1}$ (Glucose Uptake 2.83 mmol/gDWh)', fontsize=14)
    ax.set_ylabel("Frequency (Cell Population)", fontsize=14)
