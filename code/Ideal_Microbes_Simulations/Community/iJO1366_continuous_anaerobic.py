# Using the E. coli iJO1366 metabolic model under anaerobic conditions in continuous culture
# ???
...
pot_batch = -0.556
pot_const = -0.912
exp_pot_default = np.array([[0.76, 0.67304348, 0.76, 0.71652174, 0.76, 0.76, 0]])
exp_pot_fitted = np.array([0.76, 0.67304348, 0.76, 0.71652174, 0.76, 0.76, pot_const]) # pot_batch or pot_const
...
def_exp_pot = microbe.exp_pot
print(f"default expression potential: {def_exp_pot}")
...
for idx1, idx2, name1, name2 in substrate_pairs:
    ...
    plt.savefig(output_dir / f"{filename_base}_const.png", dpi=300)
    plt.savefig(output_dir / f"{filename_base}_const.svg")
    plt.close()
...
for idx1, idx2, name1, name2 in substrate_pairs:
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f"iJO1366_{name2.lower()}_{name1.lower()}_microbegrowth_const.png", dpi=300)
    plt.savefig(output_dir / f"iJO1366_{name2.lower()}_{name1.lower()}_microbegrowth_const.svg")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f"iJO1366_{name2.lower()}_{name1.lower()}_nutrientlevel_const.png", dpi=300)
    plt.savefig(output_dir / f"iJO1366_{name2.lower()}_{name1.lower()}_nutrientlevel_const.svg")
    plt.close()
