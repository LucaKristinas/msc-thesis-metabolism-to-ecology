...
new_exp_pot = 0.382 + met_noise * np.log(1 + (1 / Km_glc))
exp_pot = np.array([0.75,0.75,new_exp_pot])
...
# Create the actual plot - New version: catching the third return value (vectors)

fig, ax = plt.subplots(figsize=(5, 4)) 

heatmap, contours, vectors = microbe.plot_growth_plane(
    met_ext=met_ext,
    met_ext_index=met_ext_index,
    met_ext_max=met_ext_max,
    ax=ax,
    contours=True,
    prod_cons=False, # Even if this is False, the function still returns a third value (None)
...
# Save to CSV
output_file = processed_csv_path / "E_coli_core_sim_OFVs_Glc_meta.csv"
# utput_file = processed_csv_path / "E_coli_core_sim_OFVs_GlcFrc.csv"
