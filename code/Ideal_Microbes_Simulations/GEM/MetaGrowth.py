...
class Microbe:
    ...
    def __init__(self, met_noise, mich_ment, react_rate, extreme_path, stoich_int, stoich_biomass, stoich_ext=None, stoich_cost=None, enz_total=1, exp_pot=None, met_int_steady=None, met_int_units=None, enz_units=None, fba_approach=False):
        ...
        Defines class variables of the same name and type as input, as well as additional class variables:
            ...
            - fict_extreme_path: Boolean array specifying whether the pathways are fictitious (1) or not (0) (ndarray with shape (path_num)).
        ...
        # 5f - Determine the weighting coefficients Y_zi=W_zi/k_i of each reaction i for each class z
        # Initialize the class_path matrix
        self.class_weight = np.zeros((self.class_num, self.react_num))
        # Find fictitious reactions and pathways
        fict_reaction = (np.all((stoich_int == 0), axis=0) & np.all((stoich_ext == 0), axis=0) & (stoich_biomass == 0) & (stoich_cost == 0)).astype(int) # binary vector of size react_num with 1 if all stoichiometries are zero (a fictitious reaction), 0 otherwise
        # Identify fictitious pathways - exactly one non-zero reaction and that reaction is fictitious
        exactly_one_nonzero = (np.sum(self.extreme_path != 0, axis=1) == 1) # find pathways with exactly one non-zero reaction
        reaction_indices = np.argmax(np.abs(self.extreme_path) != 0, axis=1) # identify the index of the non-zero reaction for each row
        is_reaction_fictitious = (self.extreme_path[np.arange(self.extreme_path.shape[0]), reaction_indices] != 0) & fict_reaction[reaction_indices] # check if the single reaction is fictitious
        is_reaction_fictitious = is_reaction_fictitious.astype(bool)
        self.fict_extreme_path = np.zeros(self.path_num) # compute the final fict_extreme_path
        self.fict_extreme_path[exactly_one_nonzero & is_reaction_fictitious] = 1 / self.mich_ment[reaction_indices[exactly_one_nonzero & is_reaction_fictitious]]
        # Compute the expression potential weights of each path
        q = np.exp(self.exp_pot / self.met_noise)/(1+self.fict_extreme_path)
        ...
        if fba_approach:
            ...
            self.stoich_cost *= self.enz_total
        # Save the units
    ...
    def infer_monod_parameters(self,met_ext,met_ext_index, met_ext_total=None):
        ...
        :return A tuple containing 5 floats: r (Unit: num. cell divisions/time), m (Unit: num. cells degraded/time),
                ...
                K (Unit: extracellular metabolites (natural), mmol/l (fba)).
        """
    ...
    # Plot difference between Scott-Hwa laws for two different enzymes
    def plot_hwa_diff(self, prot_selection, met_ext, met_ext_index, met_ext_max, ax, met_ext_total=None, **kwargs):
        """
        Plot the differences between two Scott-Hwa equations when extracellular metabolite with met_ext_index is varied between 0 and met_ext_max,
        while the remaining metabolites are fixed at prescribed values.

        :param prot_selection: A tuple with indices for two enzyme budgets to be compared (tuple/list of two integers).
        :type prot_selection: Union[tuple,list]
        :param met_ext: Prescribed extracellular metabolite levels (ndarray with shape (self.ext_num,)). Units: extracellular metabolites.
        :type met_ext: numpy.ndarray
        :param met_ext_index: Index of the extracellular metabolite to be varied (int).
        :type met_ext_index: int
        ...
        :type ax: matplotlib.axes.Axes
        :param kwargs: optional keyword arguments for the plot function.
        :returns: curve: plotted Scott-Hwa difference curve (list of matplotlib.lines.Line2D objects).
        """
        # prepare axes
        num = 100
        ext_met_axis = np.linspace(0, met_ext_max, num=num)
        ...
        for num_i in range(num):
            ...
            # find the growth rate
            growth_axis[num_i] = self.growth_rate(enz_sat)
            exp_axis[num_i] = enz_exp[prot_selection[0]]-enz_exp[prot_selection[1]]
        # If colour not specified, use the default colour scheme
        if "color" not in kwargs:
            kwargs["color"] = self.dark_colors[met_ext_index]
        # Make the plot
